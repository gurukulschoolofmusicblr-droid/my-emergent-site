...
export default function Gallery() {
  ...
  return (
    <section id="annual-day" className="py-24 md:py-32 bg-[#FAF7F2]" data-testid="annual-day-section">
      <div className="max-w-7xl mx-auto px-6 md:px-10">
        <div className="grid lg:grid-cols-12 gap-10 items-end mb-12">
          <div className="lg:col-span-7">
            ...
            <h2 className="font-display text-5xl md:text-6xl text-[#2B1414] mt-4 leading-[0.95]">
              The stage,
              <span className="italic text-[#9C4B35]"> our students.</span>
            </h2>
          </div>
          ...
          <TabsList className="bg-transparent gap-2 p-0 mb-10 flex flex-wrap">
            {YEARS.map((y) => (
              <TabsTrigger
                ...
                className="tab-pill rounded-full border border-[#EADDD7] bg-[#FDFBF7] text-[#2B1414] px-5 py-2 text-sm hover:border-[#D95D39] transition-colors"
              >
                Annual Day {y}
              </TabsTrigger>
            ))}
                  ...
                  {GALLERY[y].map((img, i) => {
                    ...
                    return (
                      <motion.figure
                        ...
                        className={`relative overflow-hidden rounded-xl border border-[#EADDD7] group ${spans[i] || ""}`}
                      ...
                      >
                        <img
                          ...
                          loading="lazy"
                        />
                        <figcaption className="absolute inset-x-0 bottom-0 p-4 bg-gradient-to-t from-[#2B1414]/85 to-transparent text-[#FDFBF7] opacity-0 group-hover:opacity-100 transition-opacity duration-500">
                          <Badge className="bg-[#D95D39] hover:bg-[#D95D39] text-[#FDFBF7] mb-2">{img.cat}</Badge>
        ...
        <div className="mt-10 flex flex-wrap gap-2">
          {["Student Performances", "Group Choirs", "Solo Performances", "Trophy Distribution", "Parents & Students", "Grand Finale"].map((c) => (
            <span key={c} className="text-xs uppercase tracking-[0.18em] text-[#75635E] border border-[#EADDD7] rounded-full px-3 py-1">
              {c}
            </span>
          ))}
        </div>
