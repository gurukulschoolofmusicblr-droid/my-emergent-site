...
export default function Testimonials() {
  return (
    <section id="testimonials" className="py-24 md:py-32 bg-[#0E0908]" data-testid="testimonials-section">
      <div className="max-w-7xl mx-auto px-6 md:px-10">
        ...
        <Carousel opts={{ align: "start", loop: true }} className="w-full">
          <CarouselContent className="-ml-4">
            {TESTIMONIALS.map((t, i) => (
              <CarouselItem key={i} className="pl-4 md:basis-1/2 lg:basis-1/3" data-testid={`testimonial-${i}`}>
                <figure className="h-full bg-[#1A1311] border border-[#3A2A26] rounded-2xl p-8 md:p-10 flex flex-col">
          ...
          <div className="flex justify-end gap-3 mt-8">
            <CarouselPrevious className="static translate-y-0 border-[#3A2A26] text-[#F5E9D9] hover:bg-[#F5E9D9] hover:text-[#0E0908]" data-testid="testimonial-prev" />
            <CarouselNext className="static translate-y-0 border-[#3A2A26] text-[#F5E9D9] hover:bg-[#F5E9D9] hover:text-[#0E0908]" data-testid="testimonial-next" />
          </div>
        </Carousel>
      </div>
    </section>
